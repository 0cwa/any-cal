...
use any_cal_anytype_adapter::{
    AnytypeTransport, AnytypeTypedTransport, FakeAnytypeTransport, ObjectRecord, Page,
    TransportError,
};
use any_cal_app::{App, AppConfig, AppWithTransport};
use any_cal_core::{
    CompositionProfile, ProjectionPolicy, RefreshKind, ResourceEnvelope, SourceAvailability,
};
use any_cal_dav_server::Request;
use std::collections::BTreeSet;

#[derive(Clone, Debug)]
struct SpaceFaultTransport {
    inner: FakeAnytypeTransport,
    fail_shared_auth: bool,
}

impl SpaceFaultTransport {
    fn new() -> Self {
        Self {
            inner: FakeAnytypeTransport::new(32),
            fail_shared_auth: false,
        }
    }

    fn read_preflight(&self, space_id: &str) -> Result<(), TransportError> {
        if self.fail_shared_auth && space_id == "space-shared" {
...
impl AnytypeTransport for SpaceFaultTransport {
    fn list_objects(
        &mut self,
        space_id: &str,
        cursor: Option<&str>,
    ) -> Result<Page<ObjectRecord>, TransportError> {
        self.read_preflight(space_id)?;
...
impl AnytypeTypedTransport for SpaceFaultTransport {
    fn create_object_with_type(
        &mut self,
        object: ObjectRecord,
        type_key: &str,
    ) -> Result<ObjectRecord, TransportError> {
        self.inner.create_object_with_type(object, type_key)
    }
}
...
fn same_shared_contact_materializes_as_two_independent_private_person_contexts() {
    ...
    let alice = app
        .materialize_person_contexts(&profile("alice", "alice-home"), "alice-token", &scope())
        .unwrap();
    let bob = app
    ...
    app.with_transport_mut(|transport| {
        let alice_object = transport
            ...
            .unwrap();
        let mut alice_record = MaterializedReferenceRecord::from_json(&alice_object.body).unwrap();
        alice_record.reference.user_fields =
            private_person_context_fields("Alice private note", ["friend".to_owned()]);
        ...
        envelope.document.content.fields.get_mut("FN").unwrap()[0].value = "Carol Renamed".into();
    ...
    assert_eq!(
        alice[0].reference.source_fields["display_name"],
        "Carol Renamed"
    );
    assert_eq!(
        bob[0].reference.source_fields["display_name"],
        "Carol Renamed"
    );
    assert_eq!(
    ...
    assert_eq!(
        transport.create_type_keys,
        vec![
            any_cal_anytype_adapter::ANYCAL_PERSON_CONTEXT_TYPE_KEY.to_owned(),
            any_cal_anytype_adapter::ANYCAL_PERSON_CONTEXT_TYPE_KEY.to_owned(),
        ]
    );
...
#[test]
fn person_context_materialization_is_idempotent_after_reopen() {
    let mut app = App::fake(config()).unwrap();
    assert_eq!(
        app.handle(request(
            "PUT",
    ...
    let object_id = first[0].destination_object_id.clone();
    ...
    assert_eq!(second[0].destination_object_id, object_id);
...
#[test]
fn source_auth_loss_marks_person_context_unavailable_and_exact_reauth_reconnects_it() {
    let mut app =
        AppWithTransport::with_transport(config(), SpaceFaultTransport::new()).unwrap();
    assert_eq!(
        app.handle(request(
            "PUT",
            "/carddav/shared/carol.vcf",
    ...
    let first = app
        .materialize_person_contexts(&profile("alice", "alice-home"), "alice-token", &scope())
        .unwrap();
    let destination_id = first[0].destination_object_id.clone();
    let (_, account_fingerprint, source_space) = app.upstream_binding_identity("shared").unwrap();

    app.with_transport_mut(|transport| {
        let destination = transport
            .inner
            .objects
            .get_mut_in_space("space-alice", &destination_id)
            .unwrap();
        let mut record = MaterializedReferenceRecord::from_json(&destination.body).unwrap();
        record.reference.user_fields =
            private_person_context_fields("Keep through auth loss", ["private".to_owned()]);
        destination.body = record.canonical_json().unwrap();
        transport.fail_shared_auth = true;
    });
    ...
    assert!(!unavailable[0].created);
    assert_eq!(
        unavailable[0].reference.source_availability,
        SourceAvailability::Unavailable
    );
    assert_eq!(
        unavailable[0].reference.user_fields["private_notes"],
        "Keep through auth loss"
    );
    assert_eq!(
        app.upstream_binding_status("shared"),
        Some(("unknown", "unknown", "auth"))
    );

    assert!(!app.reauthorize_upstream_domain("shared", "wrong-account", &source_space));
    assert!(!app.reauthorize_upstream_domain(
        "shared",
        &account_fingerprint,
        "wrong-space"
    ));
    ...
    app.with_transport_mut(|transport| transport.fail_shared_auth = false);

    let restored = app
        .materialize_person_contexts(&profile("alice", "alice-home"), "alice-token", &scope())
        .unwrap();
    assert_eq!(restored.len(), 1);
    assert_eq!(restored[0].destination_object_id, destination_id);
    assert_eq!(restored[0].refresh_kind, RefreshKind::Reauthorized);
    assert_eq!(
        restored[0].reference.source_availability,
        SourceAvailability::Available
    );
    assert_eq!(
        restored[0].reference.user_fields["private_notes"],
        "Keep through auth loss"
    );
}
